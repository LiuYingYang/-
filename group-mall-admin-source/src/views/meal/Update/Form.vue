 ...
 * @LastEditTime: 2022-01-05 10:49:40
-->
<template>
  <div class="meal-form-container">
    <el-form ref="mealRef" class="dataForm" label-width="120px" label-position="left">
      ...
      <el-form-item label="升级套餐" v-if="dataForm.optionType === 3">
        <div class="meal-container" v-for="meal in dataList" :key="meal.id">
          <div class="meal-title" v-if="meal.openState" @click="selectMeal(meal)" :class="{
      ...
      <el-form-item label="套餐单价">
        <span v-if="renewMeal.packagePrice && dataForm.optionType === 2">{{ renewMeal.packagePrice }}元/{{
            packagePriceUnitMap[renewMeal.packagePriceUnit - 1]
          }}</span>
        <span v-if="selectedMeal.packagePrice && dataForm.optionType === 3">{{ selectedMeal.packagePrice }}元/{{
            packagePriceUnitMap[selectedMeal.packagePriceUnit - 1]
          }}</span>
      </el-form-item>
      <el-form-item label="套餐周期" v-if="dataForm.optionType === 2">
        <el-slider v-model="dataForm.buyPeriod" :min="
            minDayMap[
          ...
          " show-tooltip show-input :marks="marks">
        </el-slider>
      </el-form-item>
    </el-form>
    ...
    <div class="footer">
      ...
      <el-button type="primary" @click="payMoney" :disabled="dataForm.optionType === 3 && remainDay <= 7">确认</el-button>
    </div>

    <PayMoney :visible.sync="paymentVisible" :price="realPrice" :shop-info="shopInfo"
      :meal="dataForm.optionType === 2 ? renewMeal : selectedMeal" :form="dataForm" />
  </div>
</template>
