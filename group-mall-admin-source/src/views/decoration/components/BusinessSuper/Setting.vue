  ...
  <div class="spellPage--setting">
    <el-form ref="formData" :model="formData" label-width="80px" label-position="left">
      ...
      <div class="class__box">
        ...
        <div class="list__item" v-for="(group, index) in formData.firstCatList" :key="index" :draggable="true"
          @dragstart="handleDragClass(index)" @dragover="handleDragoverClass" @drop="handleDropClass(index)">
          {{ group.category.name }}
          <span @click="handleGoodGroupDel(group.category.id)" class="click__btn">删除</span>
        </div>
      </div>

      <!-- 添加分类图片 -->
      <div class="poster__box" v-if="formData.selectMode === 2 && formData.selectType === 2">
        <div class="tips">
        ...
        <div v-for="(item, index) in currentFirstCategory.classChart" :key="index" class="homeSwiperForm-item"
          ...
          <i class="el-icon-remove-outline remove__swiper--item" @click="deleteChartItem(index)"></i>
          <el-upload class="homeSwiperForm-item__uploader" action :show-file-list="false" :auto-upload="false"
            :on-change="handleChartChange">
            <img v-if="item.img" :src="item.img" class="homeSwiperForm-item__img" />
            <i v-else class="el-icon-plus homeSwiperForm-item__plus"></i>
          </el-upload>
          <div class="homeSwiperForm-item__right" style="padding-left: 20px; padding-top: 23px">
            <el-form :model="item" label-width="70px">
              <el-form-item label="选择分类">
                <el-select v-model="item.title" placeholder="请选择" @change="classSelectChange(item)">
                  <el-option v-for="(lable, inx) in currentFirstCategory.category
                      .showCategoryVos" :key="inx" :hidden="
                      ...
                      currentFirstCategory.classChart.find(
                        ...
                        ? true
                        : false
                    " :label="lable.name" :value="lable.id">
                  </el-option>
                </el-select>
              </el-form-item>
            </el-form>
        ...
        <el-upload class="homeSwiperForm-add" style="background-color: #fff" action multiple :auto-upload="false"
          :show-file-list="false" :on-change="chartHandleChange">
          <span>+ 添加一个分类图</span>
      ...
      <div class="poster__box" v-if="formData.selectMode === 2">
        ...
        <div v-for="(item, index) in currentFirstCategory.swiperList" :key="index"
          @click.capture="swiperItemClick(item)" class="homeSwiperForm-item" style="background-color: #fff"
          :draggable="true" @dragstart="handleDragstart(index)" @dragover="handleDragover" @drop="handleDrop(index)">
          <i class="el-icon-remove-outline remove__swiper--item" @click="deleteSwiperItem(index)"></i>
          <el-upload class="homeSwiperForm-item__uploader" action :show-file-list="false" :auto-upload="false"
        ...
        <el-upload class="homeSwiperForm-add" style="background-color: #fff" v-if="
            currentFirstCategory.swiperList &&
              currentFirstCategory.swiperList.length < 5
          " action multiple :auto-upload="false" :show-file-list="false" :on-change="handleChange">
          <span>+ 添加背景图</span>
      ...
      <div class="navigation__box" v-if="isShowNav">
        <div v-for="(nav, iex) in currentFirstCategory.navigation" :key="iex">
          <div v-if="nav.navType !== 1" class="homeSwiperForm-item" style="background-color: #fff; display: block"
            :draggable="true" @dragstart="navigationDragstart(iex)" @dragover="handleDragover"
            @drop="navigationDrop(iex)">
            <i class="el-icon-remove-outline remove__swiper--item" style="z-index: 11"
              @click="deletenavigationItem(iex)"></i>
            <el-form-item label="导航名称">
              <el-input v-model="nav.name" placeholder="请输入内容"></el-input>
            </el-form-item>
            <el-form-item label="选择分类">
              <el-select v-model="nav.class" multiple placeholder="请选择" style="width: 100%"
                @change="navSelectClass(nav)">
                <el-option v-for="(cla, p) in getCanSelectClass(nav)" :key="p" :label="cla.category.name"
      ...
      <div class="content__box">
        <el-form-item label="列表样式" v-if="formData.selectMode === 1 || formData.selectType === 1">
        ...
        <el-form-item label="商品边距" class="slider__box" v-if="!isSideClassification">
          <el-slider v-model="formData.goodsPadding" :show-tooltip="false" :show-input="true" :max="30"></el-slider>
        </el-form-item>
        <el-form-item label="字体选中">
          <div class="color__select">
            <span class="color__tips">{{ formData.fontColor }}</span>
            <el-color-picker @change="colorSelectChange('fontColor')" v-model="formData.fontColor"></el-color-picker>
          </div>
        </el-form-item>
        <el-form-item label="背景选中">
          <div class="color__select">
            <span class="color__tips">{{ formData.fontBg }}</span>
            <el-color-picker @change="colorSelectChange('fontBg')" v-model="formData.fontBg"></el-color-picker>
          </div>
        </el-form-item>
        <el-form-item label="字体未选">
          <div class="color__select">
            <span class="color__tips">{{ formData.fontNotColor }}</span>
            <el-color-picker @change="colorSelectChange('fontNotColor')" v-model="formData.fontNotColor">
            </el-color-picker>
          </div>
        </el-form-item>
        <el-form-item label="背景未选">
          <div class="color__select">
            ...
            <el-color-picker @change="colorSelectChange('fontNotBg')" v-model="formData.fontNotBg"></el-color-picker>
          </div>
        </el-form-item>
      </div>
    </el-form>
    <Category v-if="dialogShow" :dialogShow.sync="dialogShow" :formData="formData" @choose="selectedCategory"
      ref="Category" />
  </div>
</template>
...
export default class BusinessSuperSettting extends Vue {
  ...
  getCanSelectClass(nav: Navigation): Chart[] {
    ...
    const temp = [...navigation.find((i) => i.navType === 1).class];
    temp.push(...nav.class);
